...
install() {
    ...
    if ! command -v kasmvncserver >/dev/null 2>&1; then
        ...
        curl -fsSL --max-time 300 -o "$deb" \
            "https://github.com/kasmtech/KasmVNC/releases/download/v1.3.3/kasmvncserver_bookworm_1.3.3_${ARCH}.deb" 2>/dev/null || \
            curl -fsSL --max-time 300 -o "$deb" \
            "https://github.com/kasmtech/KasmVNC/releases/download/v1.3.3/kasmvncserver_jammy_1.3.3_${ARCH}.deb" 2>/dev/null || \
            err "Khong tai duoc KasmVNC"
        dpkg -i "$deb" 2>/dev/null || apt-get install -f -y > /dev/null 2>&1
        rm -f "$deb"
    fi
