...
install() {
    ...
    if ! command -v kasmvncserver >/dev/null 2>&1; then
        ...
        local codename
        for codename in bookworm jammy focal noble bullseye; do
        ...
        if [[ ! -s "$deb" ]]; then
            err "Khong tai duoc KasmVNC (khong co ban .deb phu hop cho distro nay)."
        fi
        dpkg -i "$deb" > "$INSTLOG.kasmvnc" 2>&1 || true
        apt-get install -f -y >> "$INSTLOG.kasmvnc" 2>&1 || true
        rm -f "$deb"
        if ! command -v kasmvncserver >/dev/null 2>&1; then
            echo ""
            echo "----- $INSTLOG.kasmvnc -----"
            tail -50 "$INSTLOG.kasmvnc"
            echo "----------------------------"
            err "Cai KasmVNC that bai (khong thay lenh kasmvncserver sau khi cai)."
        fi
    fi
