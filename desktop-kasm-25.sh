...
DESKEOF
    ...
    if [[ -x "$SDKMGR" ]]; then
        ...
        if [[ -f "$SDK_ROOT/emulator/emulator" ]]; then
            ...
            cat > /usr/share/applications/android-emulator.desktop <<DESKEOF2
[Desktop Entry]
Name=Android Emulator (Pixel_5)
Comment=Chay may ao Android Pixel_5
Exec=$SDK_ROOT/emulator/emulator -avd Pixel_5 -gpu swiftshader_indirect -no-snapshot
Icon=/opt/android-studio/bin/studio.png
Type=Application
